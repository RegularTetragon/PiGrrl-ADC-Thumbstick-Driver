...
manager_methods pigrrl_manager::methods;

pigrrl_manager::pigrrl_manager() {
  //Any basic initialization can be done here.
  //It is INVALID to create any devices here.
}

int pigrrl_manager::init(device_manager* ref) {
...
void pigrrl_manager::init_profile() {
  //Init some event translators

  //register events from some convenient static array.
  const event_decl* ev = &pigrrl_events[0];
  for (int i = 0; ev->name && *ev->name; ev = &pigrrl_events[++i]) {
...
//PiGrrl does not rely on file level structures
int pigrrl_manager::accept_device(struct udev* udev, struct udev_device* dev) {
  ...
  //  pigrrl_device exdev = new pigrrl_device();
  //  device_plugin implementation = pigrrl_dev;
  ...
  //  ... find the appropriate pigrrl_device exdev...
...
int pigrrl_manager::process_option(const char* name, const MGField value) {
